...
- Provides JSON endpoints for market metadata, current trading signals, historical alerts, custom alert rules, portfolio trades, P&L summaries, research dashboards, screeners, company analysis, options chains, sector rotation, and accuracy backtests.
...
- `GET /api/research-dashboard`, `/api/screener`, `/api/company-analysis`, `/api/options-chain`, and `/api/sector-rotation` provide platform-inspired research modules for market mood, curated screens, fundamentals, financial history, shareholding, peers, option-chain PCR/max-pain/straddle/IV-skew, and sector rotation.

## Live data integration notes

The current implementation ships with deterministic signal, research, custom-alert, portfolio, alert-history, market-structure, backtest, and simulation engines so it works without broker or exchange credentials. For production use, connect `src/signalEngine.js`, `src/alertEngine.js`, `src/researchEngine.js`, `src/databaseEngine.js`, `src/marketStructureEngine.js`, and `src/backtestEngine.js` to licensed data providers for exchange prices, order flow, options chains, news sentiment, historical candles, corporate actions, and macro/FX feeds. Add broker-side validation, audit logging, kill switches, walk-forward validation, out-of-sample testing, and regulatory review before any real-money use.
